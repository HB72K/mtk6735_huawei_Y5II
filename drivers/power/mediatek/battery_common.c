...
static PMU_STATUS mt_battery_CheckChargerVoltage(void)
{
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
		...
		if (BMT_status.charger_vol >= v_charger_max) {
			...
			BMT_status.bat_charging_state = CHR_ERROR;
			status = PMU_STATUS_FAIL;
		} else if ((BMT_status.charger_vol > 5800) && (BMT_status.charger_protect_status == charger_OVER_VOL)) {
			status = PMU_STATUS_FAIL;
		}
	}
...
static void mt_battery_notify_VBatTemp_check(void)
{
	...
	if ((BMT_status.temperature >= TEMP_POS_45_THRESHOLD) && (BMT_status.temperature < TEMP_POS_60_THRESHOLD)) {
		// [45, 60)
		...
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) at [60, ) (too high)\n",
				    BMT_status.temperature);
	}
#if defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
		...
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) at (, 0] (too low)\n",
				    BMT_status.temperature);
	}
#else
#ifdef BAT_LOW_TEMP_PROTECT_ENABLE
...
#if 0//def BATTERY_CDP_WORKAROUND
...
int charger_hv_detect_sw_thread_handler(void *unused)
{
	ktime_t ktime;
	kal_uint32 charging_enable;
	kal_uint32 hv_voltage = V_CHARGER_MAX_RESET*1000;
	kal_bool hv_status;
