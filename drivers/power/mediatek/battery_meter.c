...
kal_int32 g_battery_id_voltage[2] = {150000,1100000};
...
#define BATTERY_ID_CHANNEL_NUM 14   //19
...
void fgauge_get_profile_id(void)
{
 ...
 printk("[fgauge_get_profile_id]Battery adc num (%d)",BATTERY_ID_CHANNEL_NUM);
	if (ret != 0)
...
void table_init(void)
{
	...
	gFG_BATT_CAPACITY = fgauge_get_Q_max(temperature);
	gFG_BATT_CAPACITY_aging = gFG_BATT_CAPACITY;
	bm_print(BM_LOG_CRTI, "[%s]gFG_BATT_CAPACITY: %d\n", __func__, gFG_BATT_CAPACITY);
