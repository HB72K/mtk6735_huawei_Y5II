...
static int max8971_write_reg(struct i2c_client *client, u8 reg, u8 value)
{
	...
	while (i--) {
        ...
        if (ret < 0)
				pr_notice("***** max8971_write_reg error *****\n");
		else
		break;
	}
...
static int max8971_read_reg(struct i2c_client *client, u8 reg)
{
	...
	while(i--){
	 ...
	 if (ret < 0)
				pr_notice("***** max8971_read_reg error *****\n");
		else
		break;
...
{
    u8  reg_val= 0;
	pr_notice("[Max8971] set charger called\n");
...
//	 D("  __set_charger before enable \n");
	...
	if( MST_test_mode == 1){
		pr_notice("[Max8971] MST_test_mode set chgcv 4.1V");
...
int max8971_check_eoc_status(void)
{
    ...
    pr_notice("[LGE_MAX8971] eoc state!!! MAX8971_REG_DETAILS2[0x%x] = 0x%x \n", MAX8971_REG_DETAILS2, eoc_status);

    if( eoc_status == MAX8971_CHG_DTLS_DONE )
    {
        pr_notice("[LGE_MAX8971] max8971_check_eoc_status = %d \n", eoc_status);
        return 1;
    }
...
int max8971_start_charging(unsigned mA)
{
    ...
    pr_notice("[LGE_MAX8971] max8971_start_charging (%d)mA \n", mA);
...
//	charging_ic_status = POWER_SUPPLY_TYPE_USB ;
	...
	pr_notice("[max8971_start_chargin] can not find current\n");
   ...
   // reg_val = max8971_chg->pdata->int_mask=0xF3 ;
    ...
    pr_notice("[max8971] max8971_chg->pdata->chgcc: 0x%x \n", max8971_chg->pdata->chgcc);
...
//                   (max8971_chg->pdata->fchgtime<<MAX8971_FCHGTIME_SHIFT));
  //      max8971_write_reg(max8971_chg->client, MAX8971_REG_FCHGCRNT, reg_val);
//    	pr_notice("[max8971] max8971_chg->pdata->dcilmt: %d \n",i);
...
static int max8971_charger_detail_irq(int irq, void *data, u8 *val)
{
	...
	pr_notice("[max8971] max8971_charger_detail_irq called \n");
    switch (irq)
    {
...
//    D("  max8971_charger_handler\n");
	pr_notice("[max8971] max8971_charger_handler called ");
...
//    max8971_write_reg(max8971_chg->client,MAX8971_REG_CHGINT_MASK , reg_val);
    ...
    if(max8971_chg != NULL )
    {
	pr_notice(" max8971_chg pointer is not NULL\n");
     	max8971_chg->chg_online = 0;
    	__set_charger(max8971_chg, 0);
    }
    else
	pr_notice(" max8971_chg pointer is NULL\n");
...
//	charging_ic_status = POWER_SUPPLY_TYPE_USB ;
    ...
    for (i=250; i<=450; i+=50)
    {
        ...
        reg_val = ((FCHG_CURRENT(450)<<MAX8971_CHGCC_SHIFT) |
                   (max8971_chg->pdata->fchgtime<<MAX8971_FCHGTIME_SHIFT));
        max8971_write_reg(max8971_chg->client, MAX8971_REG_FCHGCRNT, reg_val);
    	pr_notice("[max8971] max8971_chg->pdata->chgcc: %d \n",i);
        udelay(200);
    }
...
int max8971_start_Factory_charging(void)
{
    ...
    pr_notice("[max8971] max8971_start_Factory_charging\n");
#ifdef _TEST_
    charging_ic_status = POWER_SUPPLY_TYPE_FACTORY;
#endif /* _TEST_ */
                                        ...
                                        union power_supply_propval *val)
{
	...
	int ret = 0;
    	int chg_dtls_val;
	pr_notice(" max8971_charger_get_property: psp= %d\n",psp);
...
{
        pr_notice(" charging_ic_status: %d\n", charging_ic_status);
			...
			const struct i2c_device_id *id)
{
	...
	pr_notice(" max8971_probe called \n");
...
*/
    /* LGE_CHANGE_E [wonhui.lee@lge.com] 2011-07-29, Add for AT%CHARGE*/
        ...
        pr_notice(" MAX8971_REG_CHGINT : %d\n", ret);
	chip->chg_online = 0;
