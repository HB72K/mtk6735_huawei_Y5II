...
static void SNB1058_CheckDevID(struct i2c_client *client)
{
    ...
    if((!i2c_check_functionality(client->adapter, I2C_FUNC_I2C))) {
        pr_notice("[SNB1058] I2C check ERROR!!\n");
    }

    tempvalue = 0;
    tempvalue = i2c_smbus_read_word_data(client,0x00);
    if(tempvalue < 0){
        pr_notice("[SNB1058] Check ID error!!\n");
    }
    else {
        pr_notice("[SNB1058]Device ID = [%02x]\n",tempvalue);
    }
}
...
static void SNB1058_SetGpio(void)
{
    ...
    pr_notice("[SNB1058]GPIO_SCL[137] Mode:%d\nGPIO_SDA[138] Mode:%d\n"
...
void main_cam_dvdd_power(int on_off)
{
    ...
    if(client==NULL){
    pr_notice("main_cam_dvdd_power client NULL\n");
        return ;
    }
...
void cam_avdd_power(int on_off)
{
    ...
    if(client==NULL){
    pr_notice("cam_avdd_power client NULL\n");
        return ;
    }
...
void cam_iovdd_power(int on_off)
{
    ...
    if(client==NULL){
    pr_notice("cam_iovdd_power client NULL\n");
        return ;
    }
...
void mtk_sensor_power(int on_off)
{
    ...
    if(client==NULL){
    pr_notice("mtk_sensor_power client NULL\n");
        return ;
    }
...
void check_snb1058_status(void){
	...
	pr_notice("[SNB1058] %s : 0x08(%d) \n",__func__, data);
	data = i2c_smbus_read_word_data(client,0x09);
	pr_notice("[SNB1058] %s : 0x09(%d) \n",__func__, data);
...
void set_charger_start_mode(CHG_TYPE chg_type)
{
    ...
    pr_notice("[SNB1058] Charging Current %dmA\n",chg_type);
...
void set_charger_factory_mode(void)
{
    ...
    pr_notice("[SNB1058] FACTORY charging :  0x%02x \n",data);
}
...
void set_charger_stop_mode(void)
{
    ...
    pr_notice("[SNB1058]charging off!\n");
...
static ssize_t store_main_cam_dvdd_value(struct device_driver *ddri, char *buf, size_t count)
{
    ...
    if(onoff) {
        ...
        pr_notice("main_cam_dvdd power on!\n");
    } else {
        main_cam_dvdd_power(OFF);
        pr_notice("main_cam_dvdd power off!\n");
    }
...
static ssize_t store_cam_avdd_value(struct device_driver *ddri, char *buf, size_t count)
{
    ...
    if(onoff) {
        cam_avdd_power(ON);
        pr_notice("cam_avdd power on!\n");
    } else {
        cam_avdd_power(OFF);
        pr_notice("cam_avdd power off!\n");
    }
    i2c_data->cam_avdd = onoff;
...
static ssize_t store_cam_iovdd_value(struct device_driver *ddri, char *buf, size_t count)
{
    ...
    if(onoff) {
        cam_iovdd_power(ON);
        pr_notice("cam_iovdd power on!\n");
    } else {
        cam_iovdd_power(OFF);
        pr_notice("cam_iovdd power off!\n");
    }
...
static ssize_t store_mtk_sensor_value(struct device_driver *ddri, char *buf, size_t count)
{
    ...
    if(onoff) {
        mtk_sensor_power(ON);
        pr_notice("mtk_sensor power on!\n");
    } else {
        mtk_sensor_power(OFF);
        pr_notice("mtk_sensor power off!\n");
    }
...
static int snb1058_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id) {
    ...
    pr_notice("[SNB1058] :: snb1058 i2c probe \n");
    ...
    //pr_notice("snb1058_i2c_client number = 0x%x.\n", snb1058_i2c_client);
    ...
    {
        pr_notice("SNB1058 create attribute err = %d\n", err);
    }
#endif /* LED_FUNC */
...
static int snb1058_i2c_remove(struct i2c_client *client)
{
    ...
    {
        pr_notice("SNB1058 delete attribute err = %d\n", err);
    }
#endif /* LED_FUNC */
...
{
    pr_notice("[SNB1058] ::  snb1058_i2c_detect\n");
...
static int snb1058_probe(struct platform_device *dev)
{
    ...
    pr_notice("[SNB1058] :: charging IC Initialization is done\n");

#if 0
    if(i2c_add_driver(&snb1058_i2c_driver))
    {
        pr_notice("[SNB1058] :: add snb1058 i2c driver error !\n");
        return -1;
    }
#endif
...
static int __init snb1058_driver_init(void) {
    ...
    pr_notice("SNB1058 driver init!!\n");

    ret = i2c_register_board_info(1, &i2c_snb1058, 1);
    if(ret)
    {
        pr_notice("failed to i2c register driver. (%d) \n",ret);
        return ret;
    } else {
        pr_notice("success to i2c register driver. (%d) \n",ret);
    }
    ret = platform_driver_register(&snb1058_driver);
    if(ret)
    {
        pr_notice("failed to register driver. (%d) \n",ret);
        return ret;
    } else {
        pr_notice("success to register driver. (%d) \n",ret);
    }
   if(i2c_add_driver(&snb1058_i2c_driver))
    {
        pr_notice("[SNB1058] :: add snb1058 i2c driver error !\n");
