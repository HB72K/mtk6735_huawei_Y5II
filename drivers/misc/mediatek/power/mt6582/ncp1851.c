...
kal_uint32 ncp1851_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    pr_notice("[ncp1851_read_interface] Reg[%x]=0x%x\n", RegNum, ncp1851_reg);

    ncp1851_reg &= (MASK << SHIFT);
    *val = (ncp1851_reg >> SHIFT);
    pr_notice("[ncp1851_read_interface] Val=0x%x\n", *val);
...
kal_uint32 ncp1851_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    ret = ncp1851_read_byte(RegNum, &ncp1851_reg);
    //pr_notice("[ncp1851_config_interface] Reg[%x]=0x%x\n", RegNum, ncp1851_reg);

    ncp1851_reg &= ~(MASK << SHIFT);
    ncp1851_reg |= (val << SHIFT);

    ret = ncp1851_write_byte(RegNum, ncp1851_reg);
    //pr_notice("[ncp18516_config_interface] Write Reg[%x]=0x%x\n", RegNum, ncp1851_reg);

    // Check
    //ncp1851_read_byte(RegNum, &ncp1851_reg);
    //pr_notice("[ncp1851_config_interface] Check Reg[%x]=0x%x\n", RegNum, ncp1851_reg);
...
void ncp1851_dump_register(void)
{
    ...
    for (i=0;i<ncp1851_REG_NUM;i++)
    {
        ...
        pr_notice("[ncp1851_dump_register] Reg[0x%X]=0x%X\n", i, ncp1851_reg[i]);
    }
}
...
{
    ncp1851_read_byte(i, &ncp1851_reg[i]);
    pr_notice("[ncp1851_read_register] Reg[0x%X]=0x%X\n", i, ncp1851_reg[i]);
}
...
static int ncp1851_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    pr_notice("[ncp1851_driver_probe] \n");
...
{
    pr_notice("[show_ncp1851_access] 0x%x\n", g_reg_value_ncp1851);
...
static ssize_t store_ncp1851_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    pr_notice("[store_ncp1851_access] \n");

    if(buf != NULL && size != 0)
    {
        pr_notice("[store_ncp1851_access] buf is %s and size is %d \n",buf,size);
        reg_address = simple_strtoul(buf,&pvalue,16);

        if(size > 3)
        {
            reg_value = simple_strtoul((pvalue+1),NULL,16);
            pr_notice("[store_ncp1851_access] write ncp1851 reg 0x%x with value 0x%x !\n",reg_address,reg_value);
        ...
        else
        {
            ...
            pr_notice("[store_ncp1851_access] read ncp1851 reg 0x%x with value 0x%x !\n",reg_address,g_reg_value_ncp1851);
            pr_notice("[store_ncp1851_access] Please use \"cat ncp1851_access\" to get value\r\n");
        }
    }
    return size;
}
...
static int ncp1851_user_space_probe(struct platform_device *dev)
{
    ...
    pr_notice("******** ncp1851_user_space_probe!! ********\n" );
...
static int __init ncp1851_init(void)
{
    ...
    pr_notice("[ncp1851_init] init start\n");

    i2c_register_board_info(NCP1851_BUSNUM, &i2c_ncp1851, 1);

    if(i2c_add_driver(&ncp1851_driver)!=0)
    {
        pr_notice("[ncp1851_init] failed to register ncp1851 i2c driver.\n");
    }
    else
    {
        pr_notice("[ncp1851_init] Success to register ncp1851 i2c driver.\n");
    }

    // ncp1851 user space access interface
    ret = platform_device_register(&ncp1851_user_space_device);
    if (ret) {
        pr_notice("****[ncp1851_init] Unable to device register(%d)\n", ret);
        return ret;
    }
    ret = platform_driver_register(&ncp1851_user_space_driver);
    if (ret) {
        pr_notice("****[ncp1851_init] Unable to register driver (%d)\n", ret);
        return ret;
    }
