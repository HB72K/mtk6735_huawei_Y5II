...
static long cmdq_proc_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
	 ...
	 case DISP_IOCTL_MUTEX_CONTROL:
            ...
            CMDQ_LOG("DISP_IOCTL_MUTEX_CONTROL: %d, BLS_EN = %d\n", value, DISP_REG_GET(DISP_REG_BLS_EN));
            ...
            if(value == 1)
            {
                ...
                while(DISP_REG_GET(DISP_REG_BLS_EN) & 0x1) {
                    ...
                    count++;
                    if (count > 1000) {
                        CMDQ_ERR("fail to disable BLS (0x%x)\n", DISP_REG_GET(DISP_REG_BLS_EN));
                        break;
                    }
                }

                ncs_tuning_mode = 1;
		CMDQ_LOG("CMDQ: DISP_IOCTL_MUTEX_CONTROL , ncs_tuning_mode: %d\n", ncs_tuning_mode);
                GetUpdateMutex();
            }
            else if(value == 2)
            {
           ...
           CMDQ_LOG("DISP_IOCTL_MUTEX_CONTROL done: %d, BLS_EN = %d\n", value, DISP_REG_GET(DISP_REG_BLS_EN));
       ...
       case DISP_IOCTL_WRITE_REG:
            ...
            {
                CMDQ_ERR("DISP_IOCTL_WRITE_REG, copy_from_user failed\n");
                return -EFAULT;
            }
            ...
            {
                CMDQ_ERR("reg write, addr invalid, addr min=0x%x, max=0x%x, addr=0x%x \n",
        ...
        case DISP_IOCTL_READ_REG:
            ...
            {
                CMDQ_ERR("DISP_IOCTL_READ_REG, copy_from_user failed\n");
                return -EFAULT;
            }
            if(rParams.reg>DISPSYS_REG_ADDR_MAX || rParams.reg<DISPSYS_REG_ADDR_MIN)
            {
                CMDQ_ERR("reg read, addr invalid, addr min=0x%x, max=0x%x, addr=0x%x \n",
                    DISPSYS_REG_ADDR_MIN,
                    DISPSYS_REG_ADDR_MAX,
                    rParams.reg);
                return -EFAULT;
