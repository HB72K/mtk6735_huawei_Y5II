...
static void lcm_get_params(LCM_PARAMS *params)
{
	...
	params->dsi.ssc_disable                         = 0; //wangyang add 
	...
	*/
	...
	/* this parameter would be set to 1 if DriverIC is NTK's and when force match DSI clock for NTK's */
...
static void lcm_init(void)
{
	...
	lcm_util.set_gpio_out(GPIO_LCD_ENP , 1);

	MDELAY(1);

	lcm_util.set_gpio_out(GPIO_LCD_ENN , 1);
...
static void lcm_suspend(void)
{
	...
	SET_RESET_PIN(1);	
	MDELAY(120);
	lcm_util.set_gpio_out(GPIO_LCD_ENP , 0);
	MDELAY(10);
	lcm_util.set_gpio_out(GPIO_LCD_ENN , 0);
	MDELAY(10); 
}
...
#ifdef BUILD_LK	  
	printf("ili9881c_dsi_vdo_txd [lk]=%d %d %d \n", buffer[0],buffer[1], buffer[2]);
	printf("id =0x%x\n", id);
#else
	printk("ili9881c_dsi_vdo_txd [kernel]=%d %d %d \n", buffer[0],buffer[1], buffer[2]);
	printk("ili9881c_dsi_vdo_txd id =0x%x\n", id);
#endif
...
static unsigned int lcm_esd_check(void)
{
	...
	dsi_set_cmdq(data_array, 1, 1);  
        ...
        read_reg_v2(0x09, buffer, 4);    // A1   
	id = buffer[0]; //we only need ID     
        id1 = buffer[1]; 
        id2 = buffer[2]; 
        id3 = buffer[3]; 
        printk("ghj ########## 9806e_gp  buffer[0] = %d, buffer[1] = %d, buffer[2] =%d, buffer[3] =%d\n", buffer[0], buffer[1], buffer[2], buffer[3]);     //buffer[0] = %x, buffer[1] = %d, buffer[2] =%x\n", __func__, buffer[0], buffer[1], buffer[2]
        if(id ==0x80&&id1==0x03&&id2==0x06)//&&id3==0x00    
	{    
		return 0;   
	}
	else      
