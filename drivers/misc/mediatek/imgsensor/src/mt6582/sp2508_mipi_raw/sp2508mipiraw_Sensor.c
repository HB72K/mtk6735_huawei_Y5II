...
static void SP2508MIPI_Sensor_Init(void)
{
	...
	//add digital gain 1.377x解决高亮发粉问题 20150515
	{
		...
		SP2508MIPI_write_cmos_sensor(0x00,0x9a);
		SP2508MIPI_write_cmos_sensor(0x01,0x9a);
		SP2508MIPI_write_cmos_sensor(0x03,0x9a);
		SP2508MIPI_write_cmos_sensor(0x04,0x9a); 
...
UINT32 SP2508MIPIOpen(void)
{
    ...
    do {
	   ...
	   spin_unlock(&sp2508_drv_lock);
		if (SP2508MIPI_sensor_id == SP2508_SENSOR_ID)
			break; 
		retry--; 		
	    }while (retry > 0);
