...
#define  write_otp_reg(addr, para) iWriteReg((u16) addr , (u32) para , 1, 0x20)

kal_uint16 read_otp_reg(kal_uint32 addr)
{
	kal_uint16 get_byte=0;
    iReadReg((u16) addr ,(u8*)&get_byte,0x20);
    return get_byte;
}
...
//		return ;
	...
	{		
		...
		if((sum%255)+1 == checksum_group1){
			CAM_CALDB("jade.huang>>>imx219 awb otp in Group 1 \n");
		}
	...
	else if((((reVal)&0xF0)==0xD0))
	{
		...
		if((sum%255)+1 == checksum_group2){
			CAM_CALDB("jade.huang>>>imx219 awb otp in Group 2 \n");
		}
		awb_data[0] = 1;	//add for flag
		addr = 0x321A;
	}
	...
	{		
		...
		printk("jade.huang >>>>> imx219 awb_data[%d] = %x\n",retry,awb_data[retry]);          
	}
...
static int Check_LSC_SUM(int Group)
{
	...
	if (Group==0) //Group 1: Page2~Page3  0x3204~0x3243  , page 4:0x3204~0x3232
	{
		...
		sum_value %= 255;
		sum_value += 1;
		printk("jade.huang>>>>>sum_value=0x%x  >>>>lsc_checksum_group1=0x%x\n",sum_value,lsc_checksum_group1);
...
int IMX219_UpdateLSC()
{
	...
	if(Vaild_Group==-1)
	{
		CAM_CALDB("jade.huang>>>imx219 LSC otp Find_LSC_Group fail Vaild_Group=%d \n",Vaild_Group);
		return -1;
	}		
	...
	if(Checksum_Value == -1)
	{
		...
		if(Checksum_Value==1)
		{
			...
			CAM_CALDB("jade.huang>>>IMX219_Update Group 1 LSC OK \n");
		}			
		...
		if(Checksum_Value==1)
		{
			...
			write_otp_reg(0x0193, 0x00);
			write_otp_reg(0x01A4, 0x03);						
			CAM_CALDB("jade.huang>>>IMX219_Update Group 2 LSC OK \n");
		}			
    	   }else{
			CAM_CALDB("jade.huang>>>imx219  No Vaild LSC OTP Data\n");
			CAM_CALDB("jade.huang>>>IMX219_UpdateLSC Fail !!!!\n");                                 
		}
	}
	read_awb_data();
   return 0;
...
#endif
{
    CAM_CALDB("jade.huang1 >>>>>[EEPROM] offset \n" );
    int i4RetValue = 0;
    u8 * pBuff = NULL;
    ...
    {
	CAM_CALDB("jade.huang1 >>>>>_IOC_NONE>>>>>>>\n");
	//turn -EFAULT;
	...
	CAM_CALDB("jade.huang1 >>>>>kmalloc>>>>ok\n");
        if(NULL == pBuff)
        {
            CAM_CALDB("jade.huang >>>>>locate mem failed\n");
            return -ENOMEM;
        }
        ...
        if(_IOC_WRITE & _IOC_DIR(a_u4Command))
        {
            ...
            {    //get input structure address
                kfree(pBuff);
                CAM_CALDB("jade.huang>>>>>>y from user failed\n");
                return -EFAULT;
            }
        }
    }
    ...
    CAM_CALDB("[S24CAM_CAL] pBuff u4Length=%d.\n",ptempbuf->u4Length);
     ...
     CAM_CALDB("jade.huang>>>>>>y from user successfully\n");
    ...
    {
        kfree(pBuff);
        CAM_CALDB("[jade.huang>>>>allocate mem failed\n");
        return -ENOMEM;
    }
	CAM_CALDB("[jade.huang>>>>allocate mem successfull \n");
    ...
    if(copy_from_user((u8*)pWorkingBuff ,  (u8*)ptempbuf->pu1Params, 2))
    {
        kfree(pBuff);
        kfree(pWorkingBuff);
        CAM_CALDB("[jade.huang>>>> copy from user failed\n");
        return -EFAULT;
    } 
	
#define CAM_CALIOC_G_READ 0Xc0046905
    switch(a_u4Command)
    {
...
#endif            
            break;
	    CAM_CALDB("jade.huang >>>>>[EEPROM] offset %x \n",CAM_CALIOC_G_READ );
        case CAM_CALIOC_G_READ:
    // CAM_CALDB("[jade.huang>>>>line = %d ,a_u4Command = %x \n",__LINE__,CAM_CALIOC_G_READ);
...
{
    CAM_CALDB("jade.huang>>>>>cam_cal_open\n");
