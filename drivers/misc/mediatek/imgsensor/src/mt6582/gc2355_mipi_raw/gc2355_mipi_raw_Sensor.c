...
#include "gc2355_mipi_raw_Sensor.h"
#include "gc2355_mipi_raw_Camera_Sensor_para.h"
#include "gc2355_mipi_raw_CameraCustomized.h"
...
static void GC2355_SetDummy( const kal_uint32 iPixels, const kal_uint32 iLines )
{
	...
	GC2355DB("write iPixel=%d,iLines=%d\n",iPixels,iLines);
...
static void GC2355_Sensor_Init(void)
{
	...
	GC2355_write_cmos_sensor(0xf8,0x06); //Pll mode 2  /////86--暗处降频
	...
	GC2355_write_cmos_sensor(0x06,0x16); 
	GC2355_write_cmos_sensor(0x07,0x00);
	GC2355_write_cmos_sensor(0x08,0x18); //22
	...
	GC2355_write_cmos_sensor(0x1f,0xc8); //c8//rsgl_s_mode/vpix_s_mode 灯管横条纹
	GC2355_write_cmos_sensor(0x20,0x71);
	GC2355_write_cmos_sensor(0x21,0x20); //rsg 灯管横条纹	//////40
	...
	GC2355_write_cmos_sensor(0x2f,0x14); //[3:0]tx_width 写0能改麻点
	...
	GC2355_write_cmos_sensor(0xe4,0x01);
	GC2355_write_cmos_sensor(0xe5,0xff);//1/8 jeff
...
{
	GC2355DB("GC2355_SetShutter_by wenwu.xu,ae is closed,shutter is %x \n",iShutter); 

    //kal_uint8 i=0;
   spin_lock(&gc2355mipiraw_drv_lock);
  // iShutter=6750;                   //modified by wenwu.xu for debugging,if not use ,please diable it.
   ...
   GC2355DB("GC2355_SetShutter_by wenwu.xu,gc2355.shutter is%x \n",gc2355.shutter);
...
UINT32 GC2355Video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	GC2355DB("GC2355Video enter wenwu.xu_version:");
	#if 0
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    pSensorInfo->CaptureDelayFrame = 3;    //MODIFIED BY WENWU.XU       4
...
UINT32 GC2355SetTestPatternMode(kal_bool bEnable)
{
    ...
    if(bEnable)
    {
        GC2355_write_cmos_sensor(0x8b,0xb2);
