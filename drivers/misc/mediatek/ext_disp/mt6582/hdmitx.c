...
static HDMI_VIDEO_RESOLUTION pervious_resolution = HDMI_VIDEO_720x480p_60Hz;
...
int hdmi_rdma_address_config(bool enable, hdmi_video_buffer_info buffer_info)
{
    ...
    if (enable)
    {
        ...
        RDMAConfig(1, 
              ...
              config.pitch,				// pitch
              config.srcROI.width,		// width
              config.srcROI.height,		// height
              0,							//byte swap
              1);  
#else
...
#if defined(CONFIG_SINGLE_PANEL_OUTPUT)
            if(DISP_REG_GET(DISP_REG_CONFIG_MUTEX_MOD(dp_mutex_dst)) == 0)
            {
                HDMI_LOG(" error: mutex2 mod is 0, should set it again\n" );
                dp_mutex_dst = -1;
                return 0;
            }
#endif
...
#ifndef CONFIG_MTK_SMARTBOOK_SUPPORT
...
/*static*/ void hdmi_resume(void)
{
    ...
    if(LCM_TYPE_DPI == lcm_params->type)
    {    
        disp_bls_off_on_directly(false);
        hdmi_is_active = TRUE;
    }
    ...
    if(LCM_TYPE_DPI == lcm_params->type)
    {
        disp_bls_off_on_directly(true);
        hdmi_config_pll(pervious_resolution);
        dpi_setting_res((u8)pervious_resolution);
    ...
    switch (cmd)
    {
        case MTK_HDMI_AUDIO_VIDEO_ENABLE:
        {
            ...
            else
            {
				...
				if(true == factory_mode)
				{
				   hdmi_dpi_colorbar_enable(false);
				}
                ...
                if((LCM_TYPE_DPI == lcm_params->type) && (hdmi_is_active == true))
                {
	                hdmi_is_active = false;
                    hdmi_dpi_disable_MipiClk();
                    hdmi_config_main_disp();
        ...
        case MTK_HDMI_VIDEO_CONFIG:
        {
            ...
            pervious_resolution = (HDMI_VIDEO_RESOLUTION)arg;
            hdmi_reschange = arg;
            p->is_clock_on = false;
            atomic_set(&hdmi_rdma_update_event, 1);
            wake_up_interruptible(&hdmi_rdma_update_wq);
            ...
            hdmi_dpi_config_DualEdge(true);
#if defined(CONFIG_SINGLE_PANEL_OUTPUT)
            // before video config, should complete dpi setting, or else will casue hdcp fail
            hdmi_dpi_clk_enable(true);
            //wait 50ms to ensure dpi clk is stable
            hdmi_mdelay(50);
            hdmi_video_config(p->output_video_resolution, HDMI_VIN_FORMAT_RGB888, HDMI_VOUT_FORMAT_RGB888);
#endif
        ...
        case MTK_HDMI_SCREEN_CAPTURE:
        {
            ...
            {
                HDMI_LOG("screen_capture done,wait_times(%d)\n", capture_wait_times);
            }
