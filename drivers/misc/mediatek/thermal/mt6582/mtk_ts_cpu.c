...
/*		mtk_thermal_zone_bind_trigger_trip(thz_dev,  curr_low, 0);
		...
		if(i==0)
		{
			mtktscpu_dprintk("thermal_isr: cold interrupt error\n");
		}
		else if(i==num_trip)
...
/*		mtk_thermal_zone_bind_trigger_trip(thz_dev, curr_high, 1);
		...
		if(i==0)
		{
			...
			mtktscpu_dprintk("thermal_isr: hot interrupt error\n");
		else
...
static int _mtktscpu_set_power_consumption_state(void)
{
	...
	for(i=0; i<Num_of_OPP; i++)
	{
		if(1==cl_dev_state[i])
		{
			if(i!=previous_step)
			{
				mtktscpu_dprintk("previous_opp=%d, now_opp=%d\n", previous_step, i);
				previous_step=i;
	...
	if(i==Num_of_OPP)
	{
		if(previous_step!=-1)
		{
			...
			mtktscpu_dprintk("Free all thermal limit, previous_opp=%d\n", previous_step);
