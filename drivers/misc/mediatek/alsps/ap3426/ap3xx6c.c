...
{
	int val;
	int invalid = 1;//0 wangyang changed
...
static long ap3xx6_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ALSPS_SET_CALIBRATION_DATA:
			...
			x = (alsps_calibrate.x[0] & 0xff) | ((alsps_calibrate.x[1] & 0xff) << 8);
			unsigned int th_low = x - (atomic_read(&obj->ps_thd_val_h) - atomic_read(&obj->ps_thd_val_l));
			atomic_set(&obj->ps_thd_val_h, x);
			atomic_set(&obj->ps_thd_val_l, th_low);
			z = (alsps_calibrate.z[0] & 0xff) | ((alsps_calibrate.z[1] & 0xff) << 8);
            printk("hq_alsps_set_alsps_cali:  threshold is %d ps_thd_val_high:%d ps_thd_val_low:%d,z=%d\n"
                ,x
                ,atomic_read(&obj->ps_thd_val_h)
                ,atomic_read(&obj->ps_thd_val_l),z);
		ap3xx6_set_pcrosstalk(obj->client,z);
	...
	case ALSPS_GET_CALIBRATION_DATA:
		...
		msleep(50);
           if(!test_bit(CMC_BIT_PS, &obj->enable))
           {
			...
			z = alsps_get_corss_talk(obj, 10); // ps cross talk
			printk("kevin ps cross talk = %d\n",z);
			if(z < 511)
			{
				x = obj->hw->ps_threshold_high ; // ps threshold high
				y = obj->hw->ps_threshold_low  ; // ps threshold low
			}
			else if((z >= 511) &&(z < 700))
			{
				x = obj->hw->ps_threshold_high + z-450; // ps threshold high
				y = obj->hw->ps_threshold_low + z-450; // ps threshold low
				z = 511;
			}
			else
			{
				printk("ps cross talk was too large,calibration failed!\n");
				goto err_out;
			}
			memset(&alsps_calibrate, 0, sizeof(alsps_calibrate));
			alsps_calibrate.z[0] = z & 0xff;
			alsps_calibrate.z[1] = (z & 0xff00) >> 8;
			alsps_calibrate.x[0] = x & 0xff;
			alsps_calibrate.x[1] = (x & 0xff00) >> 8;
			alsps_calibrate.y[0] = y & 0xff;
			alsps_calibrate.y[1] = (y & 0xff00) >> 8;
			if(0 == x)
			{
			...
			atomic_set(&obj->ps_thd_val_h,  x);
			atomic_set(&obj->ps_thd_val_l,  y);
			ap3xx6_set_pcrosstalk(obj->client,z);
			printk("kevin z =%d\n",z);
			set_psensor_threshold(obj->client);
			printk("kevin after hq_alsps_get_alsps_cali: ps_thd_val_high:%d ps_thd_val_low:%d\n"
                        ,x
                        ,y);	
            break;
