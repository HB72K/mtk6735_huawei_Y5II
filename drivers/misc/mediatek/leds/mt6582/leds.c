	...
	if (pmic_type == MT65XX_LED_PMIC_LCD_ISINK)
	{
		...
		else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK2)
		{
			if((button_flag_isink2==0) && (first_time == true)) {//button flag ==0, means this ISINK is not for button backlight
				...
				upmu_set_rg_isink2_ck_sel(0);
				upmu_set_isink_ch2_mode(PMIC_PWM_0);
				upmu_set_isink_ch2_step(0x0);// 0 4mA  1 8mA   2 12mA  3 16mA 
 				//hwPWMsetting(PMIC_PWM_2, 15, 8);
		...
		else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK3)
		{
			...
			//isink0
				...
				upmu_set_isink_ch2_step(isink_cur);//16mA
			    upmu_set_rg_isink2_double_en(double_mask); // Enable double current
				//hwPWMsetting(PMIC_PWM_1, 15, 8);
				upmu_set_isink_dim2_duty(15);
				upmu_set_isink_dim2_fsel(0);//6323 1KHz
			//isink1
				...
				upmu_set_isink_ch3_step(isink_cur);//16mA
				upmu_set_rg_isink3_double_en(double_mask); // Enable double current
				//hwPWMsetting(PMIC_PWM_2, 15, 8);
				upmu_set_isink_dim3_duty(15);
				upmu_set_isink_dim3_fsel(0);//6323 1KHz
			if (level) 
	...
	switch (cust->mode) {
		...
		case MT65XX_LED_MODE_PMIC:
			...
			if((strcmp(cust->name,"sub-flashlight") == 0)) {
				switch (cust->data) {
					case MT65XX_LED_PMIC_NLED_ISINK2_3_PARALLEL:
						button_flag_isink0 = 1;
						button_flag_isink1 = 1;
						return mt_brightness_set_pmic(MT65XX_LED_PMIC_NLED_ISINK_SUBFLASH, level, bl_div_hal);
					//	return mt_brightness_set_pmic(MT65XX_LED_PMIC_NLED_ISINK3, level, bl_div_hal);
				}
			}///////////////////////////add by w5
